...
import com.google.firebase.ai.type.Type
...
object VoiceOrderParser {
    ...
    private val orderSchema = Schema.objectSchema(
        properties = mapOf(
            "items" to Schema.arraySchema(
                items = Schema.objectSchema(
                    properties = mapOf(
                        "name" to Schema.stringSchema(description = "The Tamil name of the item. Try to match closely with the shop's menu items."),
                        "quantity" to Schema.integerSchema(description = "The quantity of the item ordered (default to 1 if not specified)."),
                        "unitPrice" to Schema.doubleSchema(description = "The unit price of the item according to the shop's menu. If not on menu, estimate a reasonable price."),
                        "totalItemPrice" to Schema.doubleSchema(description = "The calculated total price for this item (quantity * unitPrice).")
                    ),
                    required = listOf("name", "quantity", "unitPrice", "totalItemPrice")
                )
            ),
            "totalPrice" to Schema.doubleSchema(description = "The sum of all totalItemPrice values."),
            "totalCount" to Schema.integerSchema(description = "The sum of all quantities of all items."),
            "itemSummaryTamil" to Schema.stringSchema(description = "A concise Tamil summary of the order, e.g. '4 பரோட்டா, 1 சிக்கன் ஃப்ரை'")
        ),
        required = listOf("items", "totalPrice", "totalCount", "itemSummaryTamil")
    )
    ...
    private val model = Firebase.ai.generativeModel(
        ...
        systemInstruction = "You are a local marketplace ordering assistant in Tamil Nadu, India. " +
